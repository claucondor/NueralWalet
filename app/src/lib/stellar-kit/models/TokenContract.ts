...
/**
 * Clase para interactuar con contratos de token SEP-41 en Soroban (versión navegador)
 */
export class TokenContract {
  ...
  /**
   * Método para obtener la cuenta origen
   */
  ...
  /**
   * Método de compatibilidad con navegador - No soportado directamente
   * @param deploymentFilePath Ruta del archivo (no funciona en navegador)
   * @param networkProvider Proveedor de red opcional
  ...
  static fromDeploymentFile(deploymentFilePath: string, networkProvider?: NetworkProvider): TokenContract {
    console.warn('fromDeploymentFile no está disponible en el navegador. Use el constructor directamente.');
    throw new Error('fromDeploymentFile no es compatible con el entorno del navegador');
  }
